%Gaussian measure on R^p (p is any positive integer - dimension is flexible here)
%SE kernel
%Lambda = 0
...
%Variables
p = 1; %Dimension we are working in.
v = 500; itr = 5; % v is no. x points. itr is no. iterations.
inc = 5; %increment between values of n
si2 = 1; %Kernel parameter - length squared
t2 = 2; %Distribution parameter - variance (tau squared) - multivariate indep gaussian
...
for j=1:itr %Each loop is approx ---- secs
    ...
    %Resampling methods - currently implemented with different choices of
    %lambda
    %Should be changed
    ...
    L = decompose_kernel(K_x); %n-DPP (function from Kulesza/Taskar's package)
    
    for n=1:inc:200
        %Monte Carlo with appropriate weights
        %This method simply draws from the underlying measure rho (called
        %'Gaussian' in the legend)
        rp = randperm(v);
...
legend('Gaussian',num2str(-coeff(1,2)),'n-DPP',num2str(-coeff(2,2)),'Resample 1',num2str(-coeff(3,2)),'Resample 0.01',num2str(-coeff(4,2)),'Location','SouthWest')
