...
%Lambda = 0.01
%(Just a special case of beta_quad_lambda.m)

tic
%Variables
v = 500; s=1; itr = 30; % v is no. x points. itr is no. iterations.
...
for j=1:itr %Each loop is approx ---- secs
    j
    x = betarnd(a,a,1,v); %Make sure script elements (e.g for loops) are in optimum order!!
    x1 = repmat(x,v,1);
    ...
    q = 1; %Importance sampling density
    ...
    K1 = K_x*inv(K_x+lambda*eye(v)); %For resampling method
    q1 = diag(K1)/trace(K1);
    ...
    for n=1:inc:200
        ...
        b = (K_x(rp(1:n),rp(1:n))+n*lambda*eye(n))\mu_x(rp(1:n))'; %Weighting by q is hidden here.
        A = h(rp(1:n))*b; %Quadrature rule
        e = (A-I)^2; %Squared error
        E(j,n,1) = e; %Write sq err into array - K(:,:,method,t) stores errors from t-th beta
        
        %n-DPP method
        Y = sample_dpp(L,n);
        b = (K_x(Y,Y)+n*lambda*diag(q2(Y)))\mu_x(Y)';
        A = h(Y)*b;
        e = (A-I)^2; %Squared error
        E(j,n,2) = e;
        ...
        b = (K_x(s1,s1)+n*lambda*diag(q1(s1)))\mu_x(s1)'; %Kernel submatrix corresponding to n points
...
for m = 1:3
    ...
    x = 0:2.5/50:2.5;
    coeff(m,:) = regress(AvE(1,101:inc:200,m)',X); %Regression coeffs.
    y = coeff(m,:)*[ones(size(x)); x];
