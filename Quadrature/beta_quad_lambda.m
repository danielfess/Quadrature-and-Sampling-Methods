...
a = .5; %Beta parameter
s = 1; %s=1 corresponds to Sobolev space containing our test functions
%s not used in code, just a note so it is clear what the RKHS is)
...
for j=1:itr %Each loop is approx ---- secs
    ...
    K1 = K_x*inv(K_x+lambda*eye(v)); %For resampling + DPP methods
    q1 = diag(K1)/trace(K1);
    
    %K2 = K_x*inv(K_x+eye(v));
    %q2 = diag(K2)/trace(K2); %Possible q for DPP case
...
legend('Beta',num2str(-coeff(1,2)),'DPP',num2str(-coeff(2,2)),'Resampling',num2str(-coeff(3,2)),'Location','SouthWest')
